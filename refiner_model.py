...
class _RefinerD(nn.Module):
    def __init__(self, nc, ndf):
        ...
        self.main = nn.Sequential(
            ...
            # Ensure input adapts to different sizes
...
class _RefinerG(nn.Module):
    ...
    def forward(self, input):
        ...
        output = F.interpolate(self.tanh(d9), size=input.shape[2:], mode='bilinear', align_corners=False)
        return output
...
# [Previous content remains exactly the same until the UnetSkipConnectionBlock class]
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downnorm = norm_layer(inner_nc) if not innermost else nn.Identity()
